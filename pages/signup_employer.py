...
from auth_utils import generate_otp, send_otp, hash_password
from database import add_user, get_user
import base64
import time

st.set_page_config(page_title="Employer Sign Up - Anvaya", layout="wide", initial_sidebar_state="collapsed")
...
st.markdown('<h3 style="text-align:center; margin-bottom:5px; font-weight:600;">Employer Sign Up</h3>', unsafe_allow_html=True)
st.markdown('<p style="text-align:center; margin-top:0px; margin-bottom:25px; font-size:15px; opacity:0.8;">Create your employer account to get started.</p>', unsafe_allow_html=True)
...
with st.form("employer_signup_form"):
    st.markdown('<p style="font-size:18px; font-weight:600; margin-bottom:20px;">Account Details</p>', unsafe_allow_html=True)
    
    name = st.text_input("Company Name", placeholder="Enter your company name", key="form_name")
    ...
    if st.session_state.otp_sent and not st.session_state.otp_verified:
        ...
        verify_clicked = st.form_submit_button("✅ Verify & Sign Up")
...
if verify_clicked and user_otp:
    if not user_otp:
        ...
        with st.spinner("Verifying OTP and creating account..."):
            ...
            add_user(name, email, hashed, "employer")
            ...
            <div style="text-align:center; padding:20px; background-color:#f0fdf4; border-radius:10px; margin:20px 0;">
                ...
                <p style="color:#166534;">Your employer account has been created successfully.</p>
