    ...
    if not conversations:
        ...
        for i, conv in enumerate(conversations):
            ...
            with col1:
                ...
                {f'<span style="background:#EF4444; color:white; padding:0.2rem 0.6rem; border-radius:40px; font-size:0.7rem; align-self:center;">{conv[9] if conv[9] > 0 else "No"} new</span>'}
                </div>
                </div>
                """, unsafe_allow_html=True)
            with col2:
