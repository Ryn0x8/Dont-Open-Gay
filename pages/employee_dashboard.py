...
if current_page == "Dashboard":
            ...
            for notif in recent_notifications[:2]:  # show latest 2
                ...
                <div class="notification-card" style="margin-bottom: 0.5rem; padding: 0.8rem;">
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        ...
                        <div>
                            <div style="font-weight: 600;">{notif[3]}</div>
                            <div style="font-size: 0.8rem; color: var(--text-light);">{notif[4]}</div>
                            <div style="font-size: 0.7rem; color: var(--text-light);">{time_str}</div>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    with col_chart1:
        ...
        if stats:
            ...
            st.info("No application status data.")
    ...
    # --- Bottom Row: Top Companies & Job Recommendations ---
                ...
                <div class="job-card" style="margin-bottom: 0.5rem;">
                    ...
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        ...
                        <div style="flex:1; height:6px; background:#e2e8f0; border-radius:4px;">
                            <div style="width:{rec['match']}%; height:6px; background:#10B981; border-radius:4px;"></div>
                        </div>
                        <span style="font-size:0.8rem; font-weight:600;">{rec['match']}%</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    # VIEW JOB PAGE
    # ---------------------------------------------------------
    if "view_job_id" in st.session_state and "job_view" in st.session_state:
        ...
        with col_pic:
            st.markdown(
                f'<div class="job-avatar">{job["company_name"][0].upper()}</div>',
                unsafe_allow_html=True
            )
        ...
        with col2:
            ...
            deadline = (
                ...
                .strftime("%Y-%m-%d")
                if job["deadline"] else "Not specified"
            )
    ...
    # ---------------------------------------------------------
        ...
        job = job_tuple_to_dict(job_tuple)
        ...
        with st.form("application_form"):
            ...
            if match_score > 0:
                ...
                <div style="width:{match_score}%;height:8px;background:{color};border-radius:4px"></div>
            ...
            if not profile[4]:
                st.warning("Upload resume in profile first")
    ...
    # ---------------------------------------------------------
        ...
        with st.expander("Filters", expanded=True):
            ...
            with c4:
                ...
                locations = sorted(
                    set(j["location"] for j in job_dicts if j["location"])
                )
        ...
        for i in range(0, len(filtered), 3):
            ...
            for j in range(3):
                ...
                with cols[j]:
                    ...
                    color = get_score_color(job["match_score"])
                    ...
                    <div style="margin-top:0.5rem;height:6px;background:#e5e7eb;border-radius:4px">

                    <div style="width:{job['match_score']}%;height:6px;background:{color};border-radius:4px"></div>
    ...
    if not conversations:
        ...
        for i, conv in enumerate(conversations):
            ...
            with col1:
                ...
                {f'<span style="background:#EF4444; color:white; padding:0.2rem 0.6rem; border-radius:40px; font-size:0.7rem; align-self:center;">{conv[9]} new</span>' if conv[9] > 0 else f'<span style="background:#8ef9ff; color:white; padding:0.2rem 0.6rem; border-radius:40px; font-size:0.7rem; align-self:center;">No new Messages</span>' }
                </div>
                </div>
                """, unsafe_allow_html=True)
            with col2:
    ...
    # --- Index constants for profile tuple (adjust based on actual database.py) ---
    # Assumed order: id, phone, location, skills, resume_path, experience_level,
    # preferred_job_type, expected_salary, bio, linkedin_url, github_url, portfolio_url,
    # projects (JSON), job_alerts_enabled
    IDX_PHONE = 1
    IDX_LOCATION = 2
    IDX_RESUME = 4
    IDX_SKILLS = 5
    ...
    with col1:
        ...
        fields = [
            bool(user[1]),  # name
            bool(profile[IDX_PHONE]),
            bool(profile[IDX_LOCATION]),
            bool(profile[IDX_SKILLS]),
            bool(profile[IDX_BIO]),
            bool(profile[IDX_RESUME]),
            bool(profile[IDX_LINKEDIN] or profile[IDX_GITHUB] or profile[IDX_PORTFOLIO]),  # any social
        ]
        # Projects count (if any)
        projects_str = profile[IDX_PROJECTS] if len(profile) > IDX_PROJECTS else ""
        if projects_str:
            try:
                projects = json.loads(projects_str)
                fields.append(len(projects) > 0)
            except:
                fields.append(False)
        else:
            fields.append(False)
        ...
        <div style="margin: 0.5rem 0;">
            <div style="display: flex; justify-content: space-between; font-size:0.8rem;">
                <span>{completed}/{total} fields filled</span>
                <span>{percent}%</span>
            </div>
            <div style="height:8px; background:#e2e8f0; border-radius:4px; width:100%;">
                <div style="width:{percent}%; height:8px; background:var(--primary); border-radius:4px;"></div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Clear upload state if no file
        if not uploaded_file:
            for key in ["uploaded_resume", "uploaded_resume_name", "show_autofill_buttons", "goodness_feedback", "last_resume_text"]:
        ...
        new_alert = st.checkbox("Receive email notifications for matching jobs", value=job_alerts)
        if new_alert != job_alerts:
            update_profile(user_id, job_alerts_enabled=new_alert)
            st.rerun()
